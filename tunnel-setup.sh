  ...
  select_interface() {
    ...
    # Coletar interfaces e IPs (compatível com Linux e macOS)
    local ifaces=()
    local ips=()
    local idx=0
    ...
    if command -v ip &>/dev/null; then
      ...
      while IFS= read -r line; do
        if [[ "$line" =~ ^[0-9]+:\ ([^:]+): ]]; then
          ...
          [ ${#ips[@]} -gt 0 ] && ips[$((idx-1))]="${BASH_REMATCH[1]}"
        fi
      done < <(ip addr 2>/dev/null)
    elif command -v ifconfig &>/dev/null; then
      # macOS / BSD: usar `ifconfig`
      local cur_iface=""
      while IFS= read -r line; do
        if [[ "$line" =~ ^([a-zA-Z0-9]+): ]]; then
          cur_iface="${BASH_REMATCH[1]}"
          [[ "$cur_iface" == "lo"* ]] && cur_iface="" && continue
          ifaces+=("$cur_iface")
          ips+=("")
          idx=$((idx + 1))
        elif [[ -n "$cur_iface" && "$line" =~ inet\ ([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+) ]]; then
          [ ${#ips[@]} -gt 0 ] && ips[$((idx-1))]="${BASH_REMATCH[1]}"
        fi
      done < <(ifconfig 2>/dev/null)
    else
      warn "Não foi possível listar interfaces (ip/ifconfig não encontrado)."
      info "O túnel usará todas as interfaces por padrão (0.0.0.0)."
      BIND_IFACE=""; LOCAL_IP=""
      return
    fi

    if [ ${#ifaces[@]} -eq 0 ]; then
      warn "Nenhuma interface de rede encontrada além de loopback."
      BIND_IFACE=""; LOCAL_IP=""
      return
    fi

    # Exibir tabela
    printf "  ${BOLD}%-4s %-16s %-18s %s${RESET}
  " "Nº" "Interface" "IP" "Estado"
    printf "  ${BLUE}%-4s %-16s %-18s %s${RESET}
  " "──" "─────────────" "───────────────" "──────"
    ...
    for i in "${!ifaces[@]}"; do
      ...
      # Detectar estado (up/down) — Linux
      local estado="${YELLOW}?${RESET}"
      if command -v ip &>/dev/null; then
        local flags
        flags=$(ip link show "$iface" 2>/dev/null)
        if echo "$flags" | grep -q "state UP"; then
          estado="${GREEN}UP${RESET}"
        elif echo "$flags" | grep -q "state DOWN"; then
          estado="${RED}DOWN${RESET}"
        fi
      fi

      # Destacar interfaces conhecidas
      local label=""
      case "$iface" in
        eth*|en*)  label="${CYAN}(cabeada)${RESET}" ;;
        wlan*|wl*) label="${CYAN}(Wi-Fi)${RESET}" ;;
        tun*|tap*) label="${YELLOW}(VPN/túnel)${RESET}" ;;
        docker*|br-*|veth*) label="${YELLOW}(container)${RESET}" ;;
      esac

      printf "  ${BOLD}%-4s${RESET} %-16s %-18s %-10b %b
  "       "[$num]" "$iface" "$ip" "$estado" "$label"
    done
  ...
  "
    ...
    if [[ -z "$escolha" || "$escolha" == "0" ]]; then
      ...
      LOCAL_IP="${ips[$sel]}"
      if [ -z "$LOCAL_IP" ]; then
        warn "Interface ${BOLD}${BIND_IFACE}${RESET} não tem IP atribuído."
        warn "O túnel pode falhar — certifique-se de que ela está ativa."
      else
        ok "Interface selecionada: ${BOLD}${BIND_IFACE}${RESET} — IP: ${BOLD}${LOCAL_IP}${RESET}"
      fi
    else
